...
MatrixXf loadPLYSimple_bin(const char* fileName, int withNormals)
{
	...
	std::cout << "ģ�͵�����" << numVertices << "  ������״̬��" << withNormals << std::endl;
...
MatrixXf samplePCByQuantization(MatrixXf pc, Vector2f& xrange, Vector2f& yrange, Vector2f& zrange, float sampleStep,  int weightByCenter)
{
	...
	float xr = xrange[1] - xrange[0];//x�Ŀ��
	...
	scene_map.resize((xnumSamplesDim + 1)*(ynumSamplesDim + 1)*(znumSamplesDim + 1));//��������
	...
	{
		const int xCell = (int)((float)xnumSamplesDim*(pc(i, 0) - xrange[0]) / xr);//����x�������±�
		const int yCell = (int)((float)ynumSamplesDim*(pc(i, 1) - yrange[0]) / yr);//����y�������±�
		const int zCell = (int)((float)znumSamplesDim*(pc(i, 2) - zrange[0]) / zr);//����z�������±�
		const int index = xCell*ynumSamplesDim*znumSamplesDim + yCell*znumSamplesDim + zCell;//�����ڶ�ά�������е��±�

		scene_map[index].push_back(i);//���±�ѹ���ά������
									  //  }
	}
	//std::cout << numSamplesDim << "  vvvv " << map.size() << std::endl;
	//std::cin >> m;
	...
	{
		numPoints += (scene_map[i].size()>0);//�����ά�������з��������
	}
	//std::cout << numPoints;
	//int ii;
	//std::cin >>ii;
	...
	for (unsigned int i = 0; i<scene_map.size(); i++)
	{
		...
		std::vector<int> curCell = scene_map[i];//���ÿ����Ԫ���ڵ�������б�
		int cn = (int)curCell.size();//�б���С
		if (cn>0)
		{
			if (weightByCenter)//�м�Ȩ����
			{
				int xCell, yCell, zCell;
				double xc, yc, zc;
				double weightSum = 0;
		
				zCell = i % znumSamplesDim;//������Ƶ���±�����
				yCell = ((i - zCell) / znumSamplesDim) % ynumSamplesDim;
				xCell = ((i - zCell - yCell*ynumSamplesDim) / (ynumSamplesDim*znumSamplesDim));
				xc = ((double)xCell + 0.5) * (double)xr / xnumSamplesDim + (double)xrange[0];//���㵥Ԫ����������
	...
	//std::cout << c << "  �²���ȥ������Ϊ0  " << numPoints << std::endl;
